...
uses
  ...
  Recebimento, System.DateUtils, System.StrUtils;
...
uses
  ...
  uformVendaParcelamento, uformCartaoParcelamento, Conexao, Nfe;
...
begin
  ...
  TAuthService.Venda.MovimentoId:= TAuthService.Terminal.Movimento.Id;
  TAuthService.Venda.Competencia:= Now();
  ...
  try
    ...
    for I:= 0 to Pred(TAuthService.Venda.Itens.Count) do
      TAuthService.Venda.Itens.Items[I].save();

    vRecebimento:= nil;
    for I:= 0 to Pred(TAuthService.Venda.Recebimentos.Count) do
    begin
      TAuthService.Venda.Recebimentos.Items[I].VendaId:= TAuthService.Venda.Id;
      TAuthService.Venda.Recebimentos.Items[I].save();

      if TAuthService.Venda.Recebimentos.Items[I].Tpag = '01' then
      begin
        vRecebimento:= TRecebimento.Create;
        vRecebimento.ContaId:= TAuthService.Terminal.ContaId;
        vRecebimento.PessoaId:= TAuthService.Venda.PessoaId;
        vRecebimento.CategoriaId:= TAuthService.Terminal.CategoriaId;
        vRecebimento.VendaId:= TAuthService.Venda.Id;
        vRecebimento.Descricao:= 'DINHEIRO - VENDA ' + TAuthService.Venda.Referencia.ToString();
        vRecebimento.Competencia:= Now;
        vRecebimento.Valor:= (TAuthService.Venda.Recebimentos.Items[I].Recebido - TAuthService.Venda.Recebimentos.Items[I].Troco);
        vRecebimento.Vencimento:= Now;
        vRecebimento.Recebimento:= Now;
        vRecebimento.Situacao:= 'F';
        vRecebimento.save();
      end
      else if TAuthService.Venda.Recebimentos.Items[I].Tpag = '02' then
      begin
        vRecebimento:= TRecebimento.Create;
        vRecebimento.ContaId:= TAuthService.Terminal.ContaId;
        vRecebimento.PessoaId:= TAuthService.Venda.PessoaId;
        vRecebimento.CategoriaId:= TAuthService.Terminal.CategoriaId;
        vRecebimento.VendaId:= TAuthService.Venda.Id;
        vRecebimento.Descricao:= 'CHEQUE - VENDA ' + TAuthService.Venda.Referencia.ToString();
        vRecebimento.Competencia:= Now;
        vRecebimento.Valor:= (TAuthService.Venda.Recebimentos.Items[I].Recebido - TAuthService.Venda.Recebimentos.Items[I].Troco);
        vRecebimento.Vencimento:= Now;
        vRecebimento.Recebimento:= Now;
      ...
      else if TAuthService.Venda.Recebimentos.Items[I].Tpag = '05' then
      begin
        vRecebimento:= TRecebimento.Create;
        vRecebimento.ContaId:= TAuthService.Terminal.ContaId;
        vRecebimento.PessoaId:= TAuthService.Venda.PessoaId;
        vRecebimento.CategoriaId:= TAuthService.Terminal.CategoriaId;
        vRecebimento.VendaId:= TAuthService.Venda.Id;
        vRecebimento.Descricao:= 'CREDITO LOJA - VENDA ' + TAuthService.Venda.Referencia.ToString();
        vRecebimento.Competencia:= Now;
        vRecebimento.Valor:= (TAuthService.Venda.Recebimentos.Items[I].Recebido - TAuthService.Venda.Recebimentos.Items[I].Troco);
        vRecebimento.Vencimento:= EndOfTheMonth(now);
        vRecebimento.Situacao:= 'A';
        vRecebimento.save();
      end
      else if TAuthService.Venda.Recebimentos.Items[I].Tpag = '14' then
      begin
        for J:= 0 to Pred(TAuthService.lParcelas.Count) do
        begin
          TAuthService.lParcelas.Items[I].ContaId:= TAuthService.Terminal.ContaId;
          TAuthService.lParcelas.Items[I].PessoaId:= TAuthService.Venda.PessoaId;
          TAuthService.lParcelas.Items[I].CategoriaId:= TAuthService.Terminal.CategoriaId;
          TAuthService.lParcelas.Items[I].VendaId:= TAuthService.Venda.Id;
          TAuthService.lParcelas.Items[I].Descricao:= (I + 1).ToString() + '/' + TAuthService.lParcelas.Count.ToString() + ' - VENDA ' + TAuthService.Venda.Referencia.ToString();
          TAuthService.lParcelas.Items[I].Situacao:= 'A';
          TAuthService.lParcelas.Items[I].save();
        end;
      end
      else if TAuthService.Venda.Recebimentos.Items[I].Tpag = '15' then
      begin
        for J:= 0 to Pred(TAuthService.lParcelas.Count) do
        begin
          TAuthService.lParcelas.Items[I].ContaId:= TAuthService.Terminal.ContaId;
      ...
      else if TAuthService.Venda.Recebimentos.Items[I].Tpag = '90' then
      begin
        vRecebimento:= TRecebimento.Create;
        vRecebimento.ContaId:= TAuthService.Terminal.ContaId;
        vRecebimento.PessoaId:= TAuthService.Venda.PessoaId;
        vRecebimento.CategoriaId:= TAuthService.Terminal.CategoriaId;
        vRecebimento.VendaId:= TAuthService.Venda.Id;
        vRecebimento.Descricao:= 'OUTRO - VENDA ' + TAuthService.Venda.Referencia.ToString();
        vRecebimento.Competencia:= Now;
        vRecebimento.Valor:= (TAuthService.Venda.Recebimentos.Items[I].Recebido - TAuthService.Venda.Recebimentos.Items[I].Troco);
        vRecebimento.Vencimento:= Now;
        vRecebimento.Recebimento:= Now;
    ...
    begin
      ...
      begin
        TAuthService.listCtRecebimento.Items[I].PessoaId:= TAuthService.Venda.PessoaId;
        TAuthService.listCtRecebimento.Items[I].VendaId:= TAuthService.Venda.Id;
            ...
            TAuthService.listCtRecebimento.Items[I].Parcela.ToString() + '/' +
              TAuthService.listCtRecebimento.Items[I].QtdeParcelas.ToString() + ' - ' +
                TAuthService.listCtRecebimento.Items[I].Cartao.Nome +
                  ' CREDITO' + ' - VENDA ' + TAuthService.Venda.Referencia.ToString();
        end
        else
        begin
          TAuthService.listCtRecebimento.Items[I].Descricao:=
            TAuthService.listCtRecebimento.Items[I].Cartao.Nome +
              IfThen(vCtModalidade in[0,2], ' DEBITO', '') + ' - VENDA ' + TAuthService.Venda.Referencia.ToString();
        end;
    ...
    begin
      try
        try
          TAuthService.NfeId:= TNfe.vendaToNFCe(TAuthService.Venda.Id);
    ...
    else if THelper.Mensagem('Deseja imprimir o comprovante?', 1) then
      TVenda.imprimir(TAuthService.Venda.Id);
    ...
    13: begin
      ...
      case Pagamento of
        ...
        3: begin
          ...
          if TAuthService.Venda.Pessoa.Documento = '00000000000' then
          begin
            THelper.Mensagem('Cliente identificado como consumidor.');
            Exit();
          end;
...
begin
  ...
  if TAuthService.Venda.Pessoa.Documento = '00000000000' then
  begin
    pnl_pessoa.Caption:= 'CLIENTE IDENTIFICADO COMO CONSUMIDOR';
    pnl_pessoa.Font.Color:= clRed;
  end
  else
  begin
    pnl_pessoa.Caption:= TAuthService.Venda.Pessoa.Nome;
    pnl_pessoa.Font.Color:= clGreen;
  end;
end;
