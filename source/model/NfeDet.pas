...
uses
  ...
  NfeDetIcms, NfeDetIpi, NfeDetIi, NfeDetPis, NfeDetPisSt, NfeDetCofins,
  NfeDetCofinsSt, NfeDetIssqn, NfeDetIcmsUfDest, Item, System.StrUtils,
  UnidadeConversao;
...
type
  ...
  private
    ...
    FCOFINSST: TNfeDetCofinsSt;
    FISSQN: TNfeDetIssqn;
    FICMSUFDest: TNfeDetIcmsUfDest;

    function getITEM: TItem;
    function getCONVERSOR: TUnidadeConversao;
    function getICMS: TNfeDetIcms;
    function getIPI: TNfeDetIpi;
    function getII: TNfeDetIi;
    function getPIS: TNfeDetPis;
    function getPISST: TNfeDetPisSt;
    function getCOFINS: TNfeDetCofins;
    function getCOFINSST: TNfeDetCofinsSt;
    function getISSQN: TNfeDetIssqn;
    function getICMSUFDest: TNfeDetIcmsUfDest;
  ...
  public
    ...
    procedure ICMSUFDestCreate();
    procedure IPICreate();
    procedure IICreate();
    procedure PISCreate();
    procedure PISSTCreate();
    procedure COFINSCreate();
    procedure COFINSSTCreate();
    procedure processaCalculos();
    procedure rememberItem();
    ...
    class function findByNfeRecebidaId(NfeId, search: string): TObjectList<TNfeDet>; overload;
    class procedure findByNfeRecebidaId(NfeId, search: string; DataSet: TFDMemTable); overload;
    ...
    property CONVERSOR: TUnidadeConversao read getCONVERSOR;
    property ICMS: TNfeDetIcms read getICMS;
    property IPI: TNfeDetIpi read getIPI;
    property II: TNfeDetIi read getII;
    property PIS: TNfeDetPis read getPIS;
    property PISST: TNfeDetPisSt read getPISST;
    property COFINS: TNfeDetCofins read getCOFINS;
    property COFINSST: TNfeDetCofinsSt read getCOFINSST;
    property ISSQN: TNfeDetIssqn read getISSQN;
    property ICMSUFDest: TNfeDetIcmsUfDest read getICMSUFDest;
...
uses
  AuthService, ItemCodigo, UnidadeConversaoCodigo;

{ TNfeDet }

procedure TNfeDet.autoAjuste;
var
  FDQuery: TFDQuery;
  vSql: string;
  vParticipanteId: string;
  vCodigo: string;
begin
  try
    ...
    try
      if (Self.ItemId <> EmptyStr) and
        (Self.UnidadeConversaoId <> EmptyStr) then
      begin
        Self.Rollback;
        Exit();
      end;

      vSql:= 'SELECT PARTICIPANTE_ID FROM NFES WHERE ID = :ID';
      FDQuery.SQL.Clear;
      ...
      if not (FDQuery.RecordCount >= 1) then
      begin
        Self.Rollback;
        Exit();
      end;

      vParticipanteId:= FDQuery.FieldByName('PARTICIPANTE_ID').AsString;

      if (Self.ItemId = EmptyStr) then
      begin
        vCodigo:= vParticipanteId + '-' + Self.Cprod;
        Self.ItemId:= TItemCodigo.find(vCodigo);
        if (Self.ItemId = EmptyStr) and (Self.Cean <> EmptyStr) then
          Self.ItemId:= TItemCodigo.find(Self.Cean);
      end;

      if (Self.ItemId <> EmptyStr) and (Self.UnidadeConversaoId = EmptyStr) then
      begin
        vCodigo:= Self.ItemId + '-' + vParticipanteId + '-' + Self.Ucom;
        Self.UnidadeConversaoId:= TUnidadeConversaoCodigo.find(vCodigo);
      end;
      ...
      begin
        Self.Rollback;
        raise Exception.Create('Falha no ajuste automático de itens. erro: ' + e.Message);
      end;
    end;
  finally
    FreeAndNil(FDQuery);
...
procedure TNfeDet.COFINSSTCreate;
begin
  if not Assigned(Self.COFINSST) then
    Self.FCOFINSST:= TNfeDetCofinsSt.Create;
end;
...
procedure TNfeDet.ICMSUFDestCreate;
begin
  if not Assigned(Self.ICMSUFDest) then
    Self.FICMSUFDest:= TNfeDetIcmsUfDest.Create;
end;

procedure TNfeDet.IICreate;
begin
  if not Assigned(Self.II) then
    Self.FII:= TNfeDetIi.Create;
end;
...
procedure TNfeDet.PISSTCreate;
begin
  if not Assigned(Self.PISST) then
    Self.FPISST:= TNfeDetPisSt.Create;
end;
...
procedure TNfeDet.rememberItem;
var
  FDQuery: TFDQuery;
  vSql: string;
  vParticipanteId: string;
...
begin
  try
    ...
    try
      ...
      if (TItemCodigo.find(vCodigo) = EmptyStr) then
      begin
        with TItemCodigo.Create do
        begin
          ItemId:= Self.ItemId;
          Codigo:= vCodigo;
          save();
      ...
      if (vCodigo <> EmptyStr) and (TItemCodigo.find(vCodigo) = EmptyStr) then
      begin
        with TItemCodigo.Create do
        begin
          ItemId:= Self.ItemId;
      ...
      begin
        vCodigo:= Self.ItemId + '-' + vParticipanteId + '-' + Self.Ucom;
        if (TUnidadeConversaoCodigo.find(vCodigo) = EmptyStr) then
        begin
          with TUnidadeConversaoCodigo.Create do
          begin
            UnidadeConversaoId:= Self.UnidadeConversaoId;
            Codigo:= vCodigo;
            save();
            Free;
          end;
      ...
      begin
        Self.Rollback;
        raise Exception.Create('Falha ao guardar dados de conversor do item. erro: ' + e.Message);
      end;
    end;
  finally
    FreeAndNil(FDQuery);
...
begin
  ...
  if Assigned(Self.FCONVERSOR) then FreeAndNil(Self.FCONVERSOR);
  if Assigned(Self.FICMS) then FreeAndNil(Self.FICMS);
  if Assigned(Self.FIPI) then FreeAndNil(Self.FIPI);
  if Assigned(Self.FII) then FreeAndNil(Self.FII);
  if Assigned(Self.FPIS) then FreeAndNil(Self.FPIS);
  if Assigned(Self.FPISST) then FreeAndNil(Self.FPISST);
  if Assigned(Self.FCOFINS) then FreeAndNil(Self.FCOFINS);
  if Assigned(Self.FCOFINSST) then FreeAndNil(Self.FCOFINSST);
  if Assigned(Self.FISSQN) then FreeAndNil(Self.FISSQN);
  if Assigned(Self.FICMSUFDest) then FreeAndNil(Self.FICMSUFDest);
...
class function TNfeDet.findByNfeRecebidaId(NfeId,
  search: string): TObjectList<TNfeDet>;
const
  FSql: string =
  'SELECT D.ID FROM NFE_DET D  ' +
  'WHERE (D.NFE_ID = :NFE_ID) ' +
  'AND (D.DELETED_AT IS NULL) ' +
  'AND (D.XPROD LIKE :SEARCH) ' +
  'ORDER BY D.NITEM';
var
  FDQuery: TFDQuery;
begin
...
class procedure TNfeDet.findByNfeRecebidaId(NfeId, search: string;
  DataSet: TFDMemTable);
var
  vList: TObjectList<TNfeDet>;
  I: Integer;
begin
  ...
  DataSet.DisableControls;
  DataSet.EmptyDataSet;
  vList:= TNfeDet.findByNfeRecebidaId(NfeId, search);
  if Assigned(vList) then
  begin
    for I := 0 to Pred(vList.Count) do
    begin
      DataSet.Append;
      vList.Items[I].autoAjuste();
      DataSet.FieldByName('ITEM_ID').AsString:= vList.Items[I].ItemId;
      DataSet.FieldByName('UNIDADE_CONVERSAO_ID').AsString:= vList.Items[I].UnidadeConversaoId;
      DataSet.FieldByName('ID').AsString:= vList.Items[I].Id;
      DataSet.FieldByName('CPROD').AsString:= vList.Items[I].Cprod;
      DataSet.FieldByName('XPROD').AsString:= vList.Items[I].Xprod;
      DataSet.FieldByName('CFOP').AsString:= vList.Items[I].Cfop;
      ...
      DataSet.FieldByName('QCOM').AsExtended:= vList.Items[I].Qcom;
      DataSet.FieldByName('VUNCOM').AsExtended:= vList.Items[I].Vuncom;
      DataSet.FieldByName('VPROD').AsExtended:= vList.Items[I].Vprod;
      DataSet.FieldByName('VFRETE').AsExtended:= vList.Items[I].Vfrete;
      DataSet.FieldByName('VSEG').AsExtended:= vList.Items[I].Vseg;
      DataSet.FieldByName('VDESC').AsExtended:= vList.Items[I].Vdesc;
      DataSet.FieldByName('VOUTRO').AsExtended:= vList.Items[I].Voutro;
      DataSet.FieldByName('VTOTAL').AsExtended:=
        vList.Items[I].Vprod + vList.Items[I].Vfrete + vList.Items[I].Vseg -
          vList.Items[I].Vdesc + vList.Items[I].Voutro;
      DataSet.Post;
    end;
    FreeAndNil(vList);
  end;
...
function TNfeDet.getCOFINSST: TNfeDetCofinsSt;
begin
  if not Assigned(Self.FCOFINSST) and (Self.Id <> EmptyStr) then
    Self.FCOFINSST:= TNfeDetCofinsSt.findByNfeDetId(Self.Id);

  Result:= Self.FCOFINSST;
end;

function TNfeDet.getCONVERSOR: TUnidadeConversao;
begin
  if not Assigned(Self.FCONVERSOR) then
    Self.FCONVERSOR:= TUnidadeConversao.find(Self.FUNIDADE_CONVERSAO_ID)
  else if Self.FCONVERSOR.Id <> Self.FUNIDADE_CONVERSAO_ID then
  begin
    FreeAndNil(FCONVERSOR);
    Self.FCONVERSOR:= TUnidadeConversao.find(Self.FUNIDADE_CONVERSAO_ID);
  end;
  Result:= Self.FCONVERSOR;
end;
...
function TNfeDet.getICMSUFDest: TNfeDetIcmsUfDest;
begin
  if not Assigned(Self.FICMSUFDest) and (Self.Id <> EmptyStr) then
    Self.FICMSUFDest:= TNfeDetIcmsUfDest.findByNfeDetId(Self.Id);

  Result:= Self.FICMSUFDest;
end;

function TNfeDet.getII: TNfeDetIi;
begin
  if not Assigned(Self.FII) and (Self.Id <> EmptyStr) then
    Self.FII:= TNfeDetIi.findByNfeDetId(Self.Id);
...
function TNfeDet.getISSQN: TNfeDetIssqn;
begin
  if not Assigned(Self.FISSQN) and (Self.Id <> EmptyStr) then
    Self.FISSQN:= TNfeDetIssqn.findByNfeDetId(Self.Id);
...
function TNfeDet.getPISST: TNfeDetPisSt;
begin
  if not Assigned(Self.FPISST) and (Self.Id <> EmptyStr) then
    Self.FPISST:= TNfeDetPisSt.findByNfeDetId(Self.Id);
...
begin
  ...
  try
    ...
    try
      ...
      if Assigned(Self.II) then
      begin
        Self.II.NfeDetId:= Self.Id;
        Self.II.save;
      end;
      ...
      if Assigned(Self.PISST) then
      begin
        Self.PISST.NfeDetId:= Self.Id;
        Self.PISST.save;
      end;
      ...
      if Assigned(Self.COFINSST) then
      begin
        Self.COFINSST.NfeDetId:= Self.Id;
        Self.COFINSST.save;
      end;

      if Assigned(Self.ISSQN) then
      begin
        Self.ISSQN.NfeDetId:= Self.Id;
        Self.ISSQN.save;
      end;

      if Assigned(Self.ICMSUFDest) then
      begin
        Self.ICMSUFDest.NfeDetId:= Self.Id;
        Self.ICMSUFDest.save;
      end;
