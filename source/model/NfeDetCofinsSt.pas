...
type
  ...
  public
    ...
    class function findByNfeDetId(NfeDetId: string): TNfeDetCofinsSt;
...
constructor TNfeDetCofinsSt.Create;
begin
  Self.Table:= 'NFE_DET_COFINS_ST';
end;

class function TNfeDetCofinsSt.findByNfeDetId(
  NfeDetId: string): TNfeDetCofinsSt;
const
  FSql: string =
  'SELECT * FROM NFE_DET_COFINS_ST WHERE (NFE_DET_ID = :NFE_DET_ID)' +
...
function TNfeDetCofinsSt.save: Boolean;
begin
  Result:= inherited;
end;

function TNfeDetCofinsSt.store: Boolean;
const
  FSql: string =
  'INSERT INTO NFE_DET_COFINS_ST (' +
  '  ID,                          ' +
  '  EMPRESA_ID,                  ' +
  '  NFE_DET_ID,                  ' +
  '  VBC,                         ' +
...
begin
  ...
  try
    ...
    begin
      Result:= False;
      raise Exception.Create('COFINS ST erro: ' + e.Message);
    end;
    end;
  finally
    FreeAndNil(FDQuery);
...
function TNfeDetCofinsSt.update: Boolean;
const
  FSql: string =
  'UPDATE NFE_DET_COFINS_ST         ' +
  'SET VBC = :VBC,                  ' +
  '    PCOFINS = :PCOFINS,          ' +
  '    QBCPROD = :QBCPROD,          ' +
  '    VALIQPROD = :VALIQPROD,      ' +
