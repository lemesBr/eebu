...
type
  ...
  public
    ...
    class function findByNfeDetId(NfeDetId: string): TNfeDetPisSt;
...
constructor TNfeDetPisSt.Create;
begin
  Self.Table:= 'NFE_DET_PIS_ST';
end;

class function TNfeDetPisSt.findByNfeDetId(NfeDetId: string): TNfeDetPisSt;
const
  FSql: string =
  'SELECT * FROM NFE_DET_PIS_ST WHERE (NFE_DET_ID = :NFE_DET_ID)' +
...
function TNfeDetPisSt.save: Boolean;
begin
  Result:= inherited;
end;

function TNfeDetPisSt.store: Boolean;
const
  FSql: string =
  'INSERT INTO NFE_DET_PIS_ST (' +
  '  ID,                       ' +
  '  EMPRESA_ID,               ' +
  '  NFE_DET_ID,               ' +
  '  VBC,                      ' +
...
begin
  ...
  try
    ...
    begin
      Result:= False;
      raise Exception.Create('PIS ST erro: ' + e.Message);
    end;
    end;
  finally
    FreeAndNil(FDQuery);
...
function TNfeDetPisSt.update: Boolean;
const
  FSql: string =
  'UPDATE NFE_DET_PIS_ST            ' +
  'SET VBC = :VBC,                  ' +
  '    PPIS = :PPIS,                ' +
  '    QBCPROD = :QBCPROD,          ' +
  '    VALIQPROD = :VALIQPROD,      ' +
